...
describes what users get. Pick a format with `-o`, or let pubmed choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pubmed <command> -o table   # aligned columns for reading
pubmed <command> -o jsonl   # one JSON object per line, for piping
pubmed <command> -o json    # a single JSON array
pubmed <command> -o csv     # spreadsheet friendly
pubmed <command> -o tsv     # tab-separated
pubmed <command> -o url     # just the URL column
pubmed <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pubmed <command> --fields id,title,url
```
...
```bash
pubmed <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pubmed <command>            # a table, because this is a terminal
pubmed <command> | wc -l    # JSONL, because this is a pipe
```

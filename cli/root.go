// Package cli builds the pubmed command tree on top of the pubmed library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: pubmed.DefaultConfig()}

	root := &cobra.Command{
		Use:   "pubmed",
		Short: "Search PubMed biomedical literature",
		Long: `pubmed reads PubMed through the NCBI E-utilities API.
No API key is required. It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

pubmed is an independent tool and is not affiliated with NCBI or the National Library of Medicine.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = pubmed.NewClient(a.cfg)
	return nil
}
